...
<div class="container">
    ...
    <div class="row">
        <div class="col-md-6">
            <div class="text-grey padding-text h3">時間：<strong><?php echo date('Y-m-d H:i', strtotime($event['room_open_time'])); ?> - <?php echo date('H:i', strtotime($event['room_close_time'])); ?></strong></div>
            <div class="text-grey padding-text h3">報名費：</span><span class="h2 text-blue">$<?php echo number_format($event['fees'], 0, '.', ','); ?></span><span class="text-grey">元</span></div>
        </div>
        <div class="col-md-6">
            <div class="text-grey padding-text h3">店名：<span class="h3" id="shop-name"><?php echo $event['restaurant_name']; ?></span></div>
            <div class="text-grey padding-text h3">地址：<span id="shop-addr"><?php echo $event['restaurant_address']; ?></span></div>
        </div>
    </div>
    ...
    <div class="col-md-7 col-sm-12 col-xs-12 padding-text">
        ...
        <?php elseif ( date('Y-m-d H:i:s') >= $event['enroll_open_time'] AND date('Y-m-d H:i:s') <= $event['enroll_close_time'] ): ?>
            <?php if ( $remaining_slots ): ?>
                <button class="btn btn-rb">立即報名</button>
            <?php elseif ( $remaining_waiting_slots ): ?>
                <button class="btn btn-rb">我要候補</button>
                <?php
                $data = array(
                    'order_status'  => '候補'
                    );
    ...
    <div class="row">
        <div class="col-lg-12">
            <ul class="social">
                ...
                <li><a href="http://line.naver.jp/R/msg/text/?<?php echo $event['event_title']; ?> <?php echo current_url(); ?>" title="分享文章至 Line" target="_blank"><img src="<?php echo base_url(); ?>theme/img/others/line.png" alt="line"></a></li>
            </ul>
        </div>
    </div>
    <div class="row">
